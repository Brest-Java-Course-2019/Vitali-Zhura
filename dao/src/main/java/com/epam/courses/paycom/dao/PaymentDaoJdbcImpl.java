...
public class PaymentDaoJdbcImpl implements PaymentDao{
    ...
    private static final String INSERT = "insert into payment (payerName, paymentSum, companyAccount) values " +
            "(:payerName, :paymentSum, :companyAccount)";
    private static final String CANCEL = "delete from payment where paymentId = :paymentId";
    private static final String SELECT_ALL = "select paymentId, payerName, paymentSum, companyAccount, paymentDate " +
            "from payment";
    private static final String SELECT_BY_ID = "select paymentId, payerName, paymentSum, companyAccount, paymentDate " +
            "from payment where paymentId=:paymentId";
    private static final String SELECT_BY_DATE = "select * from payment where paymentDate >= :beginDate AND " +
            "paymentDate < (Select DATEADD (day, 1, :endDate))";
